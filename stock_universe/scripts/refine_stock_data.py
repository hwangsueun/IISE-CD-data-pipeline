...
★ 데이터 소스 (2026-07-01 기준, data/raw/ 8개 파일 전부 확보 완료) ★
  - 2013주식최종.xlsx (562KB)  — 시트 '주식 섹터' = 업종 마스터. 117개 유니버스 매핑용
  - stock_financial.xlsx (937KB) — 연도별 12개 시트('2012'~'2023'), 분기 재무제표
  - index_total.xlsx (360KB) — 반기별 22개 시트('13-1'~'23-2'), 밸류에이션/재무비율
  - Fin_stock.xlsx (56.6MB) — 5년 단위 9개 시트('80-84'~'20-23'), 1980~2023 종가/
    거래량/유동주식수/시가총액/매출액/외국인·기관·개인 순매수수량 결합 데이터.
    stock_price_detail의 주 소스(가장 넓은 커버리지 + 가장 많은 컬럼).
  ...
  - stock-short-selling.xlsx (6.5MB) — 3개 시트(13-17/18-22/23), 차입공매도
    ...
    참고용으로 추가 병합만 한다.
  - tenST.xlsx (4.9MB) — 유니버스 선정 당시 참고용 반기 스냅샷. 이 스크립트에서는
    사용하지 않는다(117개 종목은 이미 SELECTED_NAMES로 확정).
